...
public class PostMessage extends RuntimeMessageAction<SlackPlatform> {
	...
	/**
	 * Constructs a new {@link PostMessage} with the provided
	 ...
	 * token to authenticate the bot and post the {@code message} to the given
	 * {@code channel}.
	 *
	 * @return {@code null}
	...
	public Object compute() throws IOException {
		ChatPostMessageRequest request = ChatPostMessageRequest.builder().token(runtimePlatform.getSlackToken())
				.channel(this.runtimePlatform.getChannelId(channel)).text(message).unfurlLinks(true).unfurlMedia(true)
				.build();
		try {

...
public class NlpjsIntentMapper {
    ...
    private List<IntentExample> createIntentExamples(@NonNull IntentDefinition intentDefinition, @NonNull List<IntentParameter> intentParameters) {
        ...
        for (String trainingSentence : intentDefinition.getTrainingSentences()) {
            intentExamples.add(createIntentExample(trainingSentence, intentDefinition, intentParameters));
        }
        return intentExamples;
    }

    private IntentExample createIntentExample(@NonNull String trainingSentence, @NonNull IntentDefinition intentDefinition,
                                              @NonNull List<IntentParameter> intentParameters) {
            ...
            for (int i = 0; i < splitTrainingSentence.length; i++) {
                ...
                for (ContextParameter parameter : intentDefinition.getParameters()) {
                    if (sentencePart.equals(parameter.getTextFragment())) {
                        ...
                        if (NlpjsHelper.getEntityCount(parameter.getEntity().getReferredEntity(), intentDefinition) > 1)

package fr.zelus.jarvis.plugins.generic_chat.module.io;
 ...
 * take care of retrieving the corresponding class in its {@code containingModule}'s sub-modules classpath and
 * constructing it.
 */
public class GenericChatIntentProvider extends IntentProvider<GenericChatModule> {
    ...
    /**
     * Constructs a new {@link GenericChatIntentProvider} with the provided {@code containingModule} and {@code
     * configuration}.
     * <p>
     ...
     *
     * @param containingModule the {@link JarvisModule} containing this {@link GenericChatIntentProvider}
     ...
     */
    public GenericChatIntentProvider(GenericChatModule containingModule, Configuration configuration) {
        ...
        for (JarvisModule subModule : this.module.getSubModules()) {
            String eventProviderQualifiedName = subModule.getClass().getPackage().getName() + ".io." +
                    concreteIntentProviderName;
            try {
                Class<? extends EventProvider> eventProviderClass = Loader.loadClass(eventProviderQualifiedName,
                        EventProvider.class);
                try {
                    eventProvider = Loader.constructEventProvider(eventProviderClass, subModule, configuration);
                ...
                /*
                 * The class has been loaded and constructed, we don't need to iterate the other sub-modules.
                 */
                break;
            } catch (JarvisException e) {
                /*
                 * Unable to load the class. This exception is thrown if the IntentProvider is not defined in the
                 * current sub-module (which is possible since all the sub-modules are iterated), or if an error
                 * occurred when constructing the IntentProvider instance.
                 * We can silently ignore this exception, and continue the iteration on sub-modules in order to find
